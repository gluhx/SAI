...
def frame_slots_page():
    ...
    # === Инициализируем ВСЕ переменные, используемые в render_template, заранее ===
    frame_data = None
    all_slots = []
    assigned_slots = []
    frame_name = ""
    ...
    selected_group = None
    tools = []
    group_number = None  # ← КЛЮЧЕВОЕ ИСПРАВЛЕНИЕ: вынесено НАРУЖУ
    ...
    if selected_frame:
        ...
        # Теперь безопасно обновляем group_number
        if selected_group:
    ...
    return render_template(
        ...
        group_number=group_number,  # ← теперь всегда определена
        tools=tools,
        role=role
    )
